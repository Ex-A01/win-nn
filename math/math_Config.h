...
#ifndef NN_MATH_MATH_CONFIG_H_
#define NN_MATH_MATH_CONFIG_H_
...
// When <tt>NN_SWITCH_ENABLE_MEMBER_NAME_SHORTCUT</tt> is defined, <tt>NN_MATH_USE_ANONYMOUS_STRUCT</tt> is enabled.
...
//#define D_BENCH_FUNC      // Use C code with API call during configuration, and use with function individual measurements.
                            // Use the fastest code with the API call when not configured, and use with API measurement.
#define D_MEASURE_SWITCH    // Switch the source when the compile switch exists in the function.
                            // Measure the target function during configuration with <tt>C_FAST</tt>.
                            // Measure the target function when not configured with <tt>C_FAST_ALGO</tt>.
