...
May point to the same vector as <span class="argument">pV</span>.
...
@param[in] isChangeTrans  Specifies whether to update the translation elements (the 4th column)
@return Returns <span class="argument">pOut</span>.
 *---------------------------------------------------------------------------
...
MTX34InverseC(MTX34* pOut, const MTX34* p)
{
    ...
    // Calculate the determinant of the upper 3x3 submatrix.
    ...
    // Calculate the inverse of the upper submatrix.
...
MTX34InvTransposeC(MTX34* pOut, const MTX34* __restrict p)
{
    ...
    // Calculate the inverse-transpose of the upper submatrix.
    ...
    // The fourth columns are normally all zero.
    m[0][3] = 0.0F;
    m[1][3] = 0.0F;
    m[2][3] = 0.0F;
...
MTX34LookAtC(MTX34* pOut, const VEC3* pCamPos, const VEC3* pCamUp, const VEC3* pTarget)
{
    ...
    // Calculate the unit target vector.
    // Use a negative value to look down (-Z) axis.
    VEC3 vLook;
    ...
    // You don't need to normalize <tt>vUp</tt>, because it is normally already unit length.
...
@param[out] pOut  Pointer to the buffer that receives the calculation results.
@param[in] pCamPos  Pointer to the vector that specifies the camera position.
@param[in] twist  Angle of rotation around the camera's line of sight. In units of degrees.
@param[in] pTarget  Pointer to the vector that specifies the fixation point for the camera.
@return Returns <span class="argument">pOut</span>.
 *---------------------------------------------------------------------------
...
MTX34LookAtC(MTX34* pOut, const VEC3* pCamPos, f32 twist, const VEC3* pTarget)
{
    ...
    // Z-direction for the camera coordinate system.
    ...
    {
        // X-direction for the camera coordinate system.
        VEC3 r(lookReverse.z, 0.0f, -lookReverse.x);

        VEC3Normalize(&lookReverse, &lookReverse);
        VEC3Normalize(&r, &r);

        // Y-direction for the camera coordinate system.
        VEC3 u;
        ...
        // Get <tt>right</tt> by rotating the r-axis and u-axis <tt>cameraTwist</tt> values a half rotation clockwise on the ru-plane.
        // Note that r.y == 0.
...
@param[out] pOut  Pointer to the buffer that receives the calculation results.
@param[in] pCamPos  Pointer to the vector that specifies the camera position.
@param[in] pCamRotate  Pointer to vector that specifies the amount of camera rotation. In units of degrees.
@return Returns <span class="argument">pOut</span>.
 *---------------------------------------------------------------------------
